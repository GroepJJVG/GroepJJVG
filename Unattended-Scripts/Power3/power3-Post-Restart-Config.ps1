...
Start-Transcript -Path "C:\Windows\Setup\Scripts\power3-Post-Restart-Config.log" -Append
...
# Complete post-deployment configuration of DHCP
Write-Host "Completing post-deployment configuration of DHCP..."
try {
    ...
    # Authorize DHCP server in AD (if not already done)
    Add-DhcpServerInDC

    Write-Host "DHCP post-deployment configuration completed successfully."
}
catch {
    Write-Error "Failed to complete DHCP post-deployment configuration: $_"
}

# Remove the scheduled task
Unregister-ScheduledTask -TaskName "Complete AD DS Setup" -Confirm:$false

Write-Host "Post-restart configuration complete, including DHCP installation and post-deployment configuration."
